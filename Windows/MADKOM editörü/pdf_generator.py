...
def _gun_adli_tarih(tarih_str):
    """'23.05.2026' → '23.05.2026 Cumartesi'  |  '23.05.2026 15:00' → '23.05.2026 Cumartesi 15:00Z'"""
    if not tarih_str or tarih_str == 'Belirtilmedi':
        return tarih_str
    try:
        parcalar = tarih_str.strip().split()
        dt = datetime.strptime(parcalar[0], '%d.%m.%Y')
        gun = TR_GUNLER[dt.weekday()]
        if len(parcalar) >= 2:
            return f"{parcalar[0]} {gun} {parcalar[1]}Z"
        return f"{parcalar[0]} {gun}"
    except Exception:
        return tarih_str
...
def _build_pdf(path, meta, layers, signature="", unvan="",
    ...
    # ── İmza ────────────────────────────────────────────────────────────
    sig_para = Paragraph(
        f"{signature}<br/>{unvan}",
        ParagraphStyle("sig", parent=bold, fontSize=10, alignment=2, spaceAfter=0))
